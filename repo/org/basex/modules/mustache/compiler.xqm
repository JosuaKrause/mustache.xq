...
declare function compiler:strictXMLcompiler() as map(*) {
  map {
    ...
    "text" := function($item as element()*) as xs:string {
      serialize(text { serialize($item/node()) })
    },
...
declare function compiler:JSONcompiler() as map(*) {
  map {
    ...
    "eval" := function($item as element()*) as xs:string {
      serialize(xquery:eval( text { $item } ))
    }
  }
};

declare function compiler:compile($parseTree as element(), $map as element(), $functions as map(*), $compiler as map(*)) as xs:string {
...
declare function compiler:error($str as xs:string, $node as element()) as xs:string {
