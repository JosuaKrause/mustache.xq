...
declare variable $parser:_START_FUN_ := 13;
...
declare function parseContent($in as xs:string?, $sd as xs:string, $ed as xs:string) as element() {
   let $r := nextToken( $in, $sd, $ed ),
       $token := $r/@token/fn:number(),
       $remain := $r/@remain/fn:string()
   return
     if($token eq $parser:_EOF_) then
       ...
       let $r1 := parseSection( $remain, $token, $sd, $ed ),
           $r2 := parseContent( $r1/@remain, $sd, $ed )
       return element multi { $r2/@remain, $r1/node(), $r2/node() }
             ...
             $token eq $parser:_START_FUN_ or
       ...
       let $r1 := parseDelim( $remain, $sd, "=" || $ed ),
           $r2 := parseContent( $r1/@remain, $r1/@start, $r1/@end )
...
declare function token($token, $in, $length) as element() {
  <token token="{$token}" value="{fn:substring($in,1,$length)}"
    remain="{fn:substring($in, $length + 1)}"/>
};

declare function nextToken($in as xs:string?, $sdelim as xs:string, $edelim as xs:string) as element() {
...
declare function error($token as element()) as node() {
  fn:error(xs:QName("parser:ERR001"), "Unexpected token: """ || $token/@value || """")
};

declare function parseSection($in as xs:string?, $n as xs:double, $sd as xs:string, $ed as xs:string) as element() {
   let $r := nextToken($in, $sd, $ed),
       $token := $r/@token/fn:number(),
       $remain := $r/@remain/fn:string()
   return
     if($token eq $parser:_STRING_) then
       let $r2 := nextToken($remain, $sd, $ed),
           $token2 := $r2/@token/fn:number(),
           $remain2 := $r2/@remain/fn:string()
       return
         if($token2 eq $parser:_END_) then
           let $r3      := parseContent($remain2, $sd, $ed),
               $r4      := nextToken($r3/@remain, $sd, $ed),
               $token4  := $r4/@token/fn:number(),
...
declare function parseETag($in as xs:string?, $n as xs:double, $sd as xs:string, $ed as xs:string) as element() {
   let $r := nextToken($in, $sd, $ed),
       $token := $r/@token/fn:number(),
       $remain := $r/@remain/fn:string()
   return
     if($token eq $parser:_STRING_) then
       let $r2 := nextToken($remain, $sd, $ed),
           $token2 := $r2/@token/fn:number(),
           $remain2 := $r2/@remain/fn:string(),
           $name := fn:normalize-space($r/@value)
       return
         if($token2 eq $parser:_END_) then
           element multi {
             ...
             else if($n eq $parser:_START_PARTIAL_) then
               element partial { attribute name { $name } }
             else if($n eq $parser:_START_FUN_) then
               element fun { attribute name { $name } }
             else
                fn:error(xs:QName("parser:ERR004"), "Invalid token: " || $n)
           }
         else error($r2)
     else error($r)
};

declare function parseDelim($in as xs:string?, $sd as xs:string, $ed as xs:string) as element() {
   let $r := nextToken($in, $sd, $ed),
       $token := $r/@token/fn:number(),
       $remain := $r/@remain/fn:string()
   return
         ...
         if($token2 eq $parser:_END_) then
           ...
           return
             if(fn:count($delims) ne 2) then
               fn:error(xs:QName("parser:ERR003"), "Invalid delimeter syntax: """ || $r/@value || """")
             else element delims {
               attribute remain { $remain2 },
