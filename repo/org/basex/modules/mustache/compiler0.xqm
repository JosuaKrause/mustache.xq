...
:)
xquery version "3.0" ;
module namespace compiler = "http://basex.org/modules/mustache/compiler0";
...
declare function compiler:compile($parseTree as element(), $map as element(), $functions as map(*)) as node()* {
  compiler:compile-intern($parseTree, $map, $functions)
};
...
declare function compiler:compile-node($node as element(), $map as element()*, $functions as map(*)) as node()* {
  typeswitch($node)
    ...
    case element(section) return
      for $curPath in compiler:unpath($map, $node/@name)/*
        return compiler:compile-intern($node, element root {$curPath}, $functions)
    (: function call :)
